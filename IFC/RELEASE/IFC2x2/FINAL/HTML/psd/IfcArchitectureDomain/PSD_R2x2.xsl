...
# Date: 1999/06/28 0.1
 ...
 -->
  ...
  <xsl:template match="/">
    <HTML>
      <HEAD>
      <TITLE>IFC 2x2 Property Set Definition Reference</TITLE>
      </HEAD>
      <BODY>
       <font face="arial"/>
       <H2>IFC <font face="Times New Roman"><b><i>2x2</i></b></font> Property Set Definition Reference</H2>
       <HR size="5"/>
	...
	<HR size="5"/>
	<font size="2">Copyright (c) 2000 - 2003 International Alliance for Interoperability</font>
      </BODY>
    </HTML>
  </xsl:template>
  ...
  <xsl:template match="PropertySetDef">
	...
	<TR VALIGN="top">
	  <TD bgcolor = "#C0C0F0" WIDTH="18%"><font size="2"/><b>Applicability</b></TD>
	  <TD bgcolor = "#FFFFFF"><font size="2"/><xsl:value-of select="Applicability"/><BR/></TD>
	</TR>
	<TR VALIGN="top">
	  <TD bgcolor = "#C0C0F0" WIDTH="18%"><font size="2"/><b>Applicable Classes</b></TD>
	  <TD bgcolor = "#FFFFFF">
	...
	<TR VALIGN="top">
	  <TD bgcolor = "#C0C0F0" WIDTH="18%"><font size="2"/><b>Applicable Type Value</b></TD>
	  <TD bgcolor = "#FFFFFF"><font size="2"/><xsl:value-of select="ApplicableTypeValue"/><BR/></TD>
	</TR>
	<TR VALIGN="top">
	...
	<TR VALIGN="top">
	  <TD bgcolor = "#C0C0F0"><font size="2"/><b>Project Code</b></TD>
	  <TD bgcolor = "#FFFFFF"><font size="2"/><xsl:value-of select="IfcVersion/@projectcode"/><BR/></TD>
	</TR>
        </TABLE>
  </xsl:template>
       ...
       <xsl:for-each select="PropertyDef">
		...
		<xsl:value-of select="Name"/>
        	...
        	<xsl:when test="PropertyType/TypeComplexProperty">
        		<td VALIGN="top"><font size="2">IfcComplexProperty</font><br/></td>
        		<td VALIGN="top"><font size="2"><b>Usage Name: </b><xsl:value-of select="PropertyType/TypeComplexProperty/@name"/></font><br/></td>
        		<td VALIGN="top">
        		<font size="2">
    ...
    <xsl:variable name="schema_name" select="../IfcVersion/@schema"/>
      <xsl:for-each select="ClassName">
		...
		<xsl:call-template name="get_lower_string">
			<xsl:with-param name="str_input" select="$schema_name"/>
		</xsl:call-template>
		/lexical/
		<xsl:call-template name="get_lower_string">
			<xsl:with-param name="str_input" select="."/>
		</xsl:call-template>
		.html</xsl:attribute><font size="2"/><xsl:value-of select="." /></a><xsl:value-of select="string(' ')" />
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template match="ValueRangeDef">
  	<br/>
  	<ul>
     <li>LowerBound: <xsl:value-of select="LowerBoundValue/@value"/></li>
     <li>UpperBound: <xsl:value-of select="UpperBoundValue/@value"/></li>
      </ul>
  </xsl:template>
  ...
  <xsl:template match="ValueDef/DefaultValue ">
  	<br/>
   	<ul>
     <li>Default Value: <xsl:value-of select="@value"/></li>
      </ul>
  </xsl:template>
